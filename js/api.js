...
const supabase = window.supabase.createClient(_cfg.SUPABASE_URL, _cfg.SUPABASE_ANON_KEY);
...
const API = {
  ...
  onAuthChange(cb) { supabase.auth.onAuthStateChange((_e, session) => cb(session?.user || null)); },
  ...
  async listRequestsForRider(riderId) {
    const { data, error } = await supabase.from('requests').select('*').eq('rider_id', riderId).order('created_at', { ascending: false });
    if (error) throw new Error(error.message);
  ...
  async listRequestsForDriver(driverId) {
    ...
    const { data: reqs, error } = await supabase.from('requests').select('*').eq('status', 'open');
    if (error) throw new Error(error.message);
    const { data: myOffers } = await supabase.from('offers').select('request_id').eq('driver_id', driverId);
  ...
  async listOffersForRequest(requestId) {
    const { data, error } = await supabase.from('offers').select('*').eq('request_id', requestId).order('created_at', { ascending: true });
    if (error) throw new Error(error.message);
  ...
  async listOffersForDriver(driverId) {
    const { data, error } = await supabase.from('offers').select('*').eq('driver_id', driverId).order('created_at', { ascending: false });
    if (error) throw new Error(error.message);
    const offers = data.map(rowToOffer);
  ...
  async acceptOffer(offerId) {
    const { data: offerRow, error: e1 } = await supabase.from('offers').select('*').eq('id', offerId).single();
    if (e1) throw new Error(e1.message);
    const offer = rowToOffer(offerRow);
    // angenommenes Angebot markieren
    const { error: e2 } = await supabase.from('offers').update({ status: 'accepted', accepted_at: new Date().toISOString() }).eq('id', offerId);
    if (e2) throw new Error(e2.message);
    // Mitbewerber zurückstellen (on_hold)
    await supabase.from('offers').update({ status: 'on_hold' })
      .eq('request_id', offer.requestId).neq('id', offerId).eq('status', 'pending');
    // Anfrage auf assigned
    await supabase.from('requests').update({ status: 'assigned', accepted_offer_id: offerId }).eq('id', offer.requestId);
    return { offer };
  },
  ...
  async cancelTrip(offerId, by) {
    const { data: offerRow, error } = await supabase.from('offers').select('*').eq('id', offerId).single();
    if (error) throw new Error(error.message);
    const offer = rowToOffer(offerRow);
    ...
    await supabase.from('offers').update({ status: 'rejected', cancelled_by: by, cancelled_at: new Date().toISOString() }).eq('id', offerId);
    await supabase.from('requests').update({ status: 'open', accepted_offer_id: null }).eq('id', offer.requestId);
    // zurückgestellte Angebote reaktivieren
    await supabase.from('offers').update({ status: 'pending' }).eq('request_id', offer.requestId).eq('status', 'on_hold');
  },
  ...
  async confirmCompletion(offerId, by) {
    ...
    await supabase.from('offers').update({ status: 'rejected' }).eq('request_id', offer.requestId).eq('status', 'on_hold');
    const patch = {};
    ...
    if (bothDone) await supabase.from('requests').update({ status: 'done' }).eq('id', offer.requestId);
  },
  ...
  async rateTrip(offerId, by, stars, comment) {
    ...
    if (by === 'rider') {
      ...
      await supabase.from('offers').update({ rating_by_rider_stars: s, rating_by_rider_comment: comment || '', rating_by_rider_at: new Date().toISOString() }).eq('id', offerId);
      await this._recalcRating(offer.driverId, 'driver', s);
    } else {
      if (offer.ratingByDriver) throw new Error('Bereits bewertet');
      await supabase.from('offers').update({ rating_by_driver_stars: s, rating_by_driver_comment: comment || '', rating_by_driver_at: new Date().toISOString() }).eq('id', offerId);
  ...
  async _recalcRating(profileId, role, newStars) {
    ...
    await supabase.from('profiles').update({ [rCol]: newRating, [tCol]: newTrips }).eq('id', profileId);
  },

  async listRatingsForDriver(driverId) {
    const { data, error } = await supabase.from('offers').select('*').eq('driver_id', driverId).not('rating_by_rider_stars', 'is', null);
    if (error) throw new Error(error.message);
    const out = [];
    for (const o of data) {
  ...
  async listRatingsForRider(riderId) {
    const { data, error } = await supabase.from('offers').select('*').not('rating_by_driver_stars', 'is', null);
    if (error) throw new Error(error.message);
    const out = [];
    for (const o of data) {
      const req = await this.getRequest(o.request_id);
  ...
  async uploadDocument(driverId, kind, file) {
    ...
    const { error } = await supabase.from('profiles').update(col).eq('id', driverId);
    if (error) throw new Error(error.message);
    return { fileName: file.name, path };
  },
  /** Zeitlich begrenzten (signierten) Link zum Ansehen eines Dokuments. */
  async getDocumentUrl(path) {
    const { data, error } = await supabase.storage.from('documents').createSignedUrl(path, 300); // 5 Min gültig
    if (error) throw new Error(error.message);
